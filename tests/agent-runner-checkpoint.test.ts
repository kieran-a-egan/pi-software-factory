...
describe("runCheckpointableAgent checkpoint-capable recovery with a scripted session", () => {
  it("returns kind='checkpoint' when the post-checkpoint recovery turn submits a valid checkpoint", async () => {
    // Checkpoint recovery: the initial prompt plus one recovery prompt.
    const ctx = createModelHarness(2, { contextWindow: 100_000 });
    const { harness } = ctx;
    ...
    try {
      ...
      harness.session.emit(makeToolExecutionEndEvent());
      ...
      expect(harness.session.registeredToolNames).toEqual([
        "submit_result",
        "submit_checkpoint",
      ]);
